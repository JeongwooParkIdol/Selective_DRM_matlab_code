...
%%%%%%%%%%%%%%%%%%%% mod %%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
alp = x1;Su = x2;w = x3;ro = x4;Ro = x5;Ri = x6;
Mb = sqrt( 3*385.82*(Ro - Ri)*alp.*Su ./ ( ro .* (w.*2.*pi/60).^2 *(Ro.^3 - Ri.^3 )));
y = -(Mb - 0.37473);
u=[0.958.*(-log((0.5.*(1+erf(-U1./sqrt(2)))))).^(1/25.508);
    5000*U2+220000;
    1000*U3+21000;
    0.28+0.02*0.5.*(1+erf(U4./sqrt(2)));
    0.5*U5+24;
    0.3*U6+8;];
g = subs(y,x_sym,[u(1) u(2) u(3) u(4) u(5) u(6)]); %여기 바꾸기 performance function
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
